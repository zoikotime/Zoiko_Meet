...
export default function MeetRoom() {
  ...
  const connectWs = useCallback(() => {
    ...
    ws.onmessage = async (e) => {
      ...
      if (data.type === 'welcome') {
        ...
        if (data.meeting) { setMeetingInfo(data.meeting); setMeetingLocked(data.meeting.locked || false) }
  ...
  const renderSpeakerView = () => {
    const isSelfSpeaker = !speakerPeer || activeSpeaker === 'self'
    ...
    return (
      <div className="room-stage speaker-layout">
        <div className="speaker-main">
          {isSelfSpeaker ? (
            <div className={'tile self spotlight' + (screenOn ? ' screen' : '') + (speakingPeers.has('self') ? ' speaking' : '')}>
            ...
            <div className="tile-annotation-wrap">
              <PeerTile peer={speakerPeer} spotlight speaking={speakingPeers.has(speakerPeer.peer_id)} />
        ...
        <div className="speaker-strip">
          ...
          {thumbnailPeers.map((p) => (
            <PeerTile key={p.peer_id} peer={p} mini speaking={speakingPeers.has(p.peer_id)} />
          ))}
        </div>
      </div>
    )
  ...
  const renderGridView = () => (
    <div className={`room-stage tiles-${Math.min(tileCount, 12)}`}>
      <div className={'tile self' + (screenOn ? ' screen' : '') + (speakingPeers.has('self') ? ' speaking' : '')}>
        {videoOn || screenOn ? <video ref={selfVideoRef} autoPlay playsInline muted />
          : <div className="tile-placeholder"><Avatar name={user.name} color={user.avatar_color} size="lg" /></div>}
      ...
      {peerList.map((p) => (
        <div key={p.peer_id} className="tile-annotation-wrap">
          <PeerTile peer={p} speaking={speakingPeers.has(p.peer_id)} />
          ...
          <div className="room-sidebar">
            ...
            {sidebar === 'chat' && (
              <div className="room-chat-composer">
                <input placeholder="Send a message to everyone" value={chatDraft} onChange={(e) => setChatDraft(e.target.value)}
                  onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); sendChat() } }} />
                <button className="primary chat-send" onClick={sendChat} disabled={!chatDraft.trim()} aria-label="Send">
...
function PeerTile({ peer, spotlight = false, mini = false, speaking = false }) {
